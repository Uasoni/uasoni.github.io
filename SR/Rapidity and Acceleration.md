...
$$\tanh^{-1}v=\int_0^t a(t)\,\mathrm dt\implies v=\tanh^{-1}\left(\int_0^t a(t)\,\mathrm dt\right)$$
which can be returned back into non-natural units using dimensional analysis:
$$\boxed{v=c\tanh^{-1}\left(\frac{1}{c}\int_0^ta(t)\,\mathrm dt \right)}$$
And this gives us an expression for the rapidity of an accelerating object:
$$\phi=\frac{1}{c}\int_0^ta(t)\,\mathrm dt$$
> [!note]
> In the derivation we have used $t$ to be the time in the accelerating frame.
## Problems
#### Time in Lab
