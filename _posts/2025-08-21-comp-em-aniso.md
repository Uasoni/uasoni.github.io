...
Notice how there are 'stretching terms' (e.g. <span class="inline-math">$\frac{1}{s_x}\frac{ \partial }{ \partial x }$</span>) and 'non-stretching terms' (e.g. <span class="inline-math">$\frac{s_z}{s_y}$</span>). In particular, stretching terms activate only when inside their respective PML regions (e.g. the z stretching occurs only in z-PML), and non-stretching terms reduce to 1 in said regions (e.g. <span class="inline-math">$s_x=s_y=1$</span> in z-PML). Hence, can eliminate all non-stretching terms:
